...
function buscarMusica(termo){
    ...
    script.id = 'itens-script';

    script.src = `https://itunes.apple.com/search?term=${termo}&media=musica&limit=15&callback=processarRespota`;

}
async function buscarMusica(termo) {
    try {
        const response = await fetch('https://itunes.apple.com/search?term=${termo}&media=musica&limit=15');
...
function exibirResultados(musicas){
    ...
    musicas.forEach(musica =>{
        ...
        const capaUrl = musica.artworkUrl1100.replace('100x100','400x400');
